...
var postageForCountry = function (country) {
  var countries = overseasCountries(),
      postageCosts = overseasData()['postage_costs'],
      postageRegion = (countries[country] || 'worldwide');

  if (country && country.length) {
    return postageCosts[postageRegion];
  } else {
    return postageCosts['uk'];
  }
};

TransactionCalculator.prototype.postageCost = function () {
  if (this.transaction.registration) {
    return postageForCountry(this.postalCountry);
  } else {
    return this.transaction['postage_cost'];
  }
};

TransactionCalculator.overseasCountries = function () {
...
TransactionCalculator.prototype._calculatePostage = function () {
  if (typeof this.transaction['postage_options'] !== 'undefined') {
    ...
    if (this.postageOption !== null) {
      postageMethod = this.transaction['postage_options'][this.postageOption];
    }
    ...
    this.totalCost += this.postageCost();
...
TransactionCalculator.prototype.calculate = function (values) {
  ...
  this.postalCountry = values['postal_country'];
